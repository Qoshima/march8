(() => {
    ...
    function setTiltFromPoint(clientX, clientY) {
        ...
        // чем меньше числа, тем спокойнее tilt
    ...
    // ===== Typing effect =====
    async function typeText(el, text, speedMs = 22) {
        el.textContent = "";
    ...
    function spawnBurst(x, y, count = 40) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            particles.push({
                ...
                shape: pick(shapes),
                // нежные цвета
                color: pick(['#ff5da2', '#7cf7d4', '#f3f4ff', '#c9ccff', '#ffd1e6'])
            });
        }
        startLoop();
    }
    ...
    function drawParticle(p) {
        ...
        if (p.shape === 'dot') {
            ...
            // простая форма лепестка (овальчик + срез)
            ctx.beginPath();
    ...
    function openCard() {
        ...
        // typing
        const text = typingLine.getAttribute('data-text') || "";
        typeText(typingLine, text, 20);

        // burst по центру карточки
        const r = card.getBoundingClientRect();
    ...
    // “Ещё эффект” — в точке кнопки
    burstBtn.addEventListener('click', () => {
        const r = burstBtn.getBoundingClientRect();
        spawnBurst(r.left + r.width / 2, r.top + r.height / 2, 45);
    });

    // Дополнительно: клик в любом месте страницы → маленький эффект
    document.addEventListener('click', (e) => {
        // чтобы не спамило при клике на кнопку "Открыть": там уже делаем burst отдельно
